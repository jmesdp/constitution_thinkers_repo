...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="consent_count" as="xs:double" select="count(//sub_section/*[not(self::subsection_name) and self::consent_of_governed])"/>
    <xsl:variable name="rights_count" as="xs:double" select="count(//sub_section/*[not(self::subsection_name) and self::inherent_rights])"/>
    <xsl:variable name="justice_count" as="xs:double" select="count(//sub_section/*[not(self::subsection_name) and self::criminal_justice])"/>
    <xsl:variable name="purpose_count" as="xs:double" select="count(//sub_section/*[not(self::subsection_name) and self::purposes_of_law])"/>
    <xsl:variable name="legal_count" as="xs:double" select="count(//sub_section/*[not(self::subsection_name) and self::legal_documents])"/>
    <xsl:variable name="legislat_count" as="xs:double" select="count(//sub_section/*[not(self::subsection_name) and self::legislative_body])"/>
    <xsl:variable name="exec_count" as="xs:double" select="count(//sub_section/*[not(self::subsection_name) and self::executive_body])"/>
    <xsl:variable name="limits_count" as="xs:double" select="count(//sub_section/*[not(self::subsection_name) and self::limits_of_power])"/>
